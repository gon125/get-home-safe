...
final class TicTacToeInteractor: PresentableInteractor<TicTacToePresentable>, TicTacToeInteractable, TicTacToePresentableListener {
    ...
    private var placedMarkerCount = 0
    private var isgameEnded: (PlayerType?)? {
        let winner = checkWinner()
        if placedMarkerCount == boardSize || winner != nil {
            return (winner)
        }
        return nil
    }
    private let boardSize = GameConstants.colCount*GameConstants.rowCount
